...
export default function Highlighter(
  ...
  const isNameExists = highLightTekst.some(t => t.name.toLowerCase() === newTaskName.toLowerCase());

  const handleAdd = () => {
    if (!newTaskName.trim()) return;

    // ❌ запрет одинаковых цветов
    if (isColorExists) return;

    // ❌ запрет одинаковых названий (необязательно, но правильно)
