...
export interface ColorResearchRef {
  commitToParent: () => void;
}

const ColorResearch = forwardRef<ColorResearchRef, {
  ...
  // 🔹 локальные хайлайты (источник правды для редактора)
  const [highliResult, setHighliResult] = useState<Highlight[]>(highlights);
  ...
  /* ---------- render highlights when LOCAL state changes ---------- */
  useEffect(() => {
    const q = quillRef.current;
    if (!q) return;

    if (!highliResult.length) {
      q.setText('');
      lastSavedRef.current = '';
      return;
    }

    const ops = highliResult.map(h => ({
      insert: h.text,
      ...(h.color
  ...
  /* ---------- auto save with debounce + diff check + cursor restore (LOCAL) ---------- */
  useEffect(() => {
    const q = quillRef.current;
    if (!q) return;
  ...
  /* ---------- commit to parent ONLY on Save ---------- */
  useImperativeHandle(ref, () => ({
    commitToParent() {
      setTasks(researchTasks);
      setHighLights(highliResult);
    },
  }));
