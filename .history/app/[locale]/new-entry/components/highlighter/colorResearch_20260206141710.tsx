...
export default function ColorResearch({
  ...
  // ✅ СИНХРОННАЯ очистка фона по цвету удаляемого тега
const clearBackgroundByTask = (task: ResearchTask) => {
  const q = quillRef.current;
  if (!q) return;

  const content = q.getContents();
  const targetHex = normalizeColor(task.color);
  const targetRgba = normalizeColor(hexToRgba(task.color, 0.6));
  ...
  const newOps = content.ops?.map(op => {
    ...
    if (typeof bg === "string") {
      const opColor = normalizeColor(bg);

      if (opColor === targetHex || opColor === targetRgba) {
        const newAttrs = { ...op.attributes };
        delete newAttrs.background;
        hasChanges = true;
        return { ...op, attributes: newAttrs };
      }
    }
  ...
  if (hasChanges && newOps) {
    const range = q.getSelection();
    q.setContents(newOps, 'user');
    if (range) q.setSelection(range.index, range.length);
  }
};
