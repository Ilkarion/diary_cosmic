...
const handleTag = (task: ResearchTask, nameBtn: string) => {
  ...
  delta.ops?.forEach((op, index) => {
    if (
      ...
      typeof op.attributes?.background === "string" &&
      hexToRgba(task.color, 0.6).includes(op.attributes.background.replace(/\s/g, ""))
    ) {
      q.formatText(
        q.getIndex(index as any),
        op.insert.length,
        "background",
        false,
        "silent"
