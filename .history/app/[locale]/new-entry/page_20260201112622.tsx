...
//---------------------------------------------------------
  ...
  //SAVE
  const handleSave = () => {
    const data = saveEditor(feel, title, date, tags, tasks, highLights, allColorTags);
    setSavedData(data);
  ...
  useEffect(() => {
    if (!savedData) return;
    renderSaved(setFeel, setTitle, setDate, setTags, setTasks, savedData, setValue, setHighLights, setAllColorTags);
  }, [savedData]);
    ...
    <div className="main-wraper-new-entry">
      <div className="header-wrapper-newEntry">
        ...
        <div className="highlighter-wrapper">
          <Highlighter highLightTekst={tasks} setHighLightTekst={setTasks} allSavedTags={allColorTags}/>
        </div>
      </div>
