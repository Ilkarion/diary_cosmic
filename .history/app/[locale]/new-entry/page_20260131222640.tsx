...
import ColorResearch, { ColorResearchRef } from "./components/highlighter/colorResearch";
...
const colorResearchRef = useRef<ColorResearchRef>(null);
  ...
  //SAVE
  const handleSave = () => {
  colorResearchRef.current?.commitToParent();
    // const highlights = getRangyBlocks(editor);
    const data = saveEditor(feel, title, date, tags, tasks, highLights);
    setSavedData(data);
    ...
    <div className="main-wraper-new-entry">
      <div className="header-wrapper-newEntry">
        ...
        <div className="highlighter-wrapper">
          <Highlighter highLightTekst={tasks} setHighLightTekst={setTasks} />
        </div>
      </div>
