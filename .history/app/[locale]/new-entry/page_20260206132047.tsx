...
import { saveEditor, renderSaved, fetchDiary, fetchAllTags } from "./logicNewEntry/functions";
...
const removeGlobalTag = (tag: string) => {
  ...
  const handleSave = () => {
    const data = saveEditor(feel, title, date, tags, tasks, 
      highLights, allColorTags, allTags, mode, setSaving);
    if(data === null) {return}
    if(mode==="create") {
      setTitle(data.title)
