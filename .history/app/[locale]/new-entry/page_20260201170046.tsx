...
export default function Page() {
  ...
  //calendar
const initialRawDate = (() => {
  if (!startData.date) return "";
  const d = new Date(startData.date); // парсим Friday, December 26, 2025
  if (!isNaN(d.getTime())) {
    return d.toISOString().split("T")[0]; // YYYY-MM-DD для input
  }
  return "";
})();

  const [rawDate, setRawDate] = useState<string>(initialRawDate);// ISO format YYYY-MM-DD
