...
const removeGlobalTag = (tag: string) => {
  ...
  //SAVE
  const handleSave = () => {
    const data = saveEditor(feel, title, date, tags, tasks, highLights, allColorTags, allTags, mode);
    if(data === null) {return}
  ...
  return (
    <div className="main-wraper-new-entry">
      ...
      />
        ...
        <button className="save-btn" onClick={handleSave} disabled={true}>Save</button>
      </div>
    </div>
  );
}
